...
```diff
[features]
+ Library:AddNeonOutline(Frame, NeonInfo), Window:SetNeonOutline(NeonInfo) — customizable neon outline (Color, Opacity, Range, Layers, Pulse, PulseSpeed)
+ Library:AnimateTitleText(Label, Options), Window:SetTitleAnimation(AnimationInfo) — animated title text (Wave color-cycle / Highlight neon-letter modes), usable on both the Window title and Loading title via WindowInfo.TitleAnimation / LoadingInfo.TitleAnimation
+ Jank! intro screen using Library:CreateLoading, with an animated title and a driven progress bar (see Example.lua)

[changes]
+ Default theme (Scheme + ThemeManager.BuiltInThemes["Default"]) switched from purple/black to a flat gray theme: BackgroundColor #181818, MainColor/OutlineColor #282828, AccentColor #a3a3a3, FontColor #ffffff
+ Default font switched from Code to RobotoMono (Templates.Window.Font, Scheme.Font, ThemeManager font fallback)
```
