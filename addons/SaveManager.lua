...
--// Rejects anything that could escape the settings directory (path separators, ".."), used for
--// file-name-only inputs like ConfigName that must never be split into subdirectories.
local function IsValidFileName(Name: string): boolean
    return typeof(Name) == "string" and (
        Trim(Name) ~= "" and
        not Name:match("^%s*$") and
        not Name:find('[<>:"|%?%*%z/\\]') and
...
local function GetConfigPath(ConfigName: string): false | string
    if not IsValidFileName(ConfigName) then
        return false
    end
