...
local Library = {
    ...
    Scheme = {
        ...
        AccentColor = Color3.fromRGB(163, 163, 163),
...
local function DeepCopyTemplate(Template: { [string]: any }): { [string]: any }
    local Copy = {}

    for k, v in Template do
        Copy[k] = if typeof(v) == "table" then DeepCopyTemplate(v) else v
    end
...
function Library:Validate(Table: { [string]: any }, Template: { [string]: any }): { [string]: any }
    if typeof(Table) ~= "table" then
        return DeepCopyTemplate(Template)
    end
...
--// Title text animation: RichText color cycling / neon letter highlight, throttled via Heartbeat. \\--
...
function Library:AnimateTitleText(Label: TextLabel, Options: { [string]: any }?)
    Options = Library:Validate(Options, Templates.TitleAnimation)
    ...
    local function GetColors(): { Color3 }
        if typeof(Anim.Colors) == "table" and #Anim.Colors > 0 then
            return Anim.Colors
        end
    ...
    local function Render()
        ...
        local CharCount = utf8.len(BaseText) or #BaseText
        local Phase = (Elapsed * Anim.Speed) % 1

        local Parts = {}
        local Index = 0
        for _, CodePoint in utf8.codes(BaseText) do
            local EscapedChar = EscapeRichText(utf8.char(CodePoint))
            local Color
            ...
            if Anim.Mode == "Highlight" then
                ...
                Color = Colors[LowerIndex]:Lerp(Colors[UpperIndex], ScaledT - math.floor(ScaledT))
            end

            table.insert(Parts, string.format('<font color="%s">%s</font>', ColorToHex(Color), EscapedChar))
            Index += 1
        end
    ...
    Library:GiveSignal(Anim.Connection)

    function Anim:SetOptions(NewOptions: { [string]: any }?)
        NewOptions = NewOptions or {}
        if NewOptions.Enabled ~= nil then self.Enabled = NewOptions.Enabled end
        if NewOptions.Mode ~= nil then self.Mode = NewOptions.Mode end
...
do
    ...
    function Funcs:AddLabel(...)
        ...
        if typeof(First) == "table" or typeof(Second) == "table" then
            ...
            Data.Visible = if Params.Visible == nil then true else Params.Visible
    ...
    function Funcs:AddButton(...)
        ...
        local function GetInfo(...)
            ...
            if typeof(First) == "table" or typeof(Second) == "table" then
                ...
                Info.Visible = if Params.Visible == nil then true else Params.Visible
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        function Dropdown:BuildDropdownList()
            ...
            local TotalLen = GetTableSize(Values)
        ...
        function Dropdown:SetDisabledValues(DisabledValues)
            if typeof(DisabledValues) ~= "table" then return end
...
function Library:CreateWindow(WindowInfo)
    ...
    local WindowTitle
    local WindowIcon
    local NeonOutlineController
    local TitleAnimationController
    local RightWrapper
    local SearchBox
    ...
    do
        ...
        NeonOutlineController = Library:AddNeonOutline(MainFrame, WindowInfo.NeonOutline)
        ...
        if WindowInfo.TitleAnimation.Enabled then
            TitleAnimationController = Library:AnimateTitleText(WindowTitle, WindowInfo.TitleAnimation)
        end
            ...
            elseif Image:match("http://") or Image:match("https://") then
                ...
                if RawFileName and Domain then
                    ...
                    if FileName then
                        ...
                        if CustomImageManagerAssets[AssetName] == nil then
                            CustomImageManager.AddAsset(AssetName, 0, Image)
                        else
                            CustomImageManager.DownloadAsset(AssetName, true)
                        end

                        BackgroundImage.Image = CustomImageManager.GetAsset(AssetName)
...
function Library:CreateLoading(LoadingInfo)
    ...
    if LoadingInfo.Icon then
        ...
        assert(Icon, "Icon must be a valid Roblox asset or a valid URL or a valid lucide icon.")
    ...
    local LoadingTitleLabel = New("TextLabel", {
        BackgroundTransparency = 1,
    ...
    assert(LoaderIcon, "LoadingIcon must be a valid Roblox asset or a valid URL or a valid lucide icon.")
