...
function Neon.Apply(NewFn, Frame, GlowAssetId, TweenService, opts)
	...
	if GlowAssetId then
		...
		if TweenService and opts.Animate ~= false then
			...
			local BreatheTween = TweenService:Create(Glow, BreatheInfo, {
				ImageTransparency = math.clamp(BaseTransparency + Amplitude, 0, 1),
			})
			BreatheTween:Play()

			-- The tween has RepeatCount = -1, so it never completes on its own - without this it
			-- would keep running forever against a destroyed Instance every time a transient
			-- element (a notification, a dialog) using this glow gets destroyed.
			Glow.Destroying:Connect(function()
				BreatheTween:Cancel()
			end)
		end
	end
...
-- ==== module: ThemeMatrixGrid ====
local ThemeMatrixGrid = (function()
...
function MatrixGrid.Build(NewFn, Parent, TweenService, opts)
	...
	if TweenService then
		...
		local ScanTween = TweenService:Create(Scanline, ScanInfo, {
			Position = UDim2.new(0, 0, 1.05, 0),
		})
		ScanTween:Play()

		-- RepeatCount = -1 never completes on its own - cancel it if the grid is ever torn down
		-- independently of the whole window (defensive; the window itself only dies on Unload).
...
-- ==== module: AnimationJuice ====
local AnimationJuice = (function()
...
-- ==== module: IntroBootConsole ====
local IntroBootConsole = (function()
...
-- ==== module: IntroBootScreen ====
local IntroBootScreen = (function()
...
local Library = {
    ...
    -- Tiered animation presets (src/Animation/Presets.lua) replace the old single flat curve.
    -- `TweenInfo`/`NotifyTweenInfo` are kept as named fields for addon/back-compat.
    AnimationPresets = AnimationPresets,
    Juice = AnimationJuice,
    TweenInfo = AnimationPresets.Micro,
    NotifyTweenInfo = AnimationPresets.Notify,
    ...
    IsLightTheme = false,
    -- Black/yellow "techy" default palette (src/Theme/Palette.lua). Cloned so reloading the
    -- library never mutates the shared module table.
    Scheme = table.clone(ThemePalette.Scheme),
...
-- `Glow`: pass true to give this outline a genuine neon glow (soft backglow image, tinted
-- AccentColor, with a slow "breathe" animation) instead of a flat colored stroke. Used sparingly
-- - notifications, dialogs, the keybind panel - so it reads as a deliberate accent, not clutter.
function Library:AddOutline(Frame: GuiObject, Glow: boolean?)
    return ThemeNeon.Apply(New, Frame, Glow and CustomImageManager.GetAsset("Glow") or nil, TweenService, {
        StrokeAlias = "OutlineColor",
        GlowAlias = "AccentColor",
    })
end
...
function Library:AddDraggableMenu(Name: string)
    ...
    Library:AddOutline(Holder, true)
...
do
    ...
    function Funcs:AddButton(...)
        ...
        local function InitEvents(Button)
            ...
            Button.Base.MouseButton1Click:Connect(function()
                ...
                -- Layout-safe click feedback (stroke flash, not scale) - these buttons share a
                -- row via HorizontalFlex.Fill, so scaling one would jitter its siblings.
                AnimationJuice.ClickFlash(Button.Stroke, TweenService, AnimationPresets.Standard, Library.Scheme.AccentColor)
    ...
    function Funcs:AddCheckbox(Idx, Info)
        ...
        function Toggle:Display()
            ...
            TweenService:Create(Label, AnimationPresets.Standard, {
                TextTransparency = Toggle.Value and 0 or 0.4,
            }):Play()
            TweenService:Create(CheckImage, AnimationPresets.Standard, {
        ...
        Button.MouseButton1Click:Connect(function()
            ...
            if Toggle.Value then
                AnimationJuice.ActivationPulse(Button, TweenService, CustomImageManager.GetAsset("Glow"), Library.Scheme.AccentColor)
            end
        end)

        for _, Connection in AnimationJuice.PressScale(Button, TweenService, AnimationPresets.Press, AnimationPresets.Release) do
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        function Toggle:Display()
            ...
            TweenService:Create(Ball, AnimationPresets.Standard, {
...
-- Runs the animated `0%[====>   ]100% | message` boot bar in the executor console (rconsole
-- when available, stepped print() fallback otherwise). Call this before/alongside
-- `Library:CreateLoading` for a matching on-screen + console boot sequence. See
-- src/Intro/BootConsole.lua for the Steps shape and options.
function Library:PrintBootBar(Steps, Opts)
    IntroBootConsole.Run(Steps, Opts)
end
...
function Library:Notify(...)
    ...
    if Data.BigIcon then
        ...
        if ParsedIcon then
            ...
            local BigIconScale = New("UIScale", { Scale = 0, Parent = BigIconLabel })
            TweenService:Create(BigIconScale, AnimationPresets.Standard, { Scale = 1 }):Play()
        end
    end
    ...
    if Data.Icon and TitleContainer then
        ...
        if ParsedIcon then
            ...
            -- Small pop-in for the title icon instead of appearing instantly with the card.
            local IconScale = New("UIScale", { Scale = 0, Parent = IconLabel })
            TweenService:Create(IconScale, AnimationPresets.Standard, { Scale = 1 }):Play()
        end
    end
    ...
    IntroBootScreen.AddGlowingLeadingEdge(New, TimerFill, CustomImageManager.GetAsset("Glow"), {
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        -- Animated show/hide for the keybind panel instead of an instant Visible flip. Kept as
        -- a method (rather than requiring callers to poke `.Visible` directly) so scripts like
        -- Example.lua get the scale+pop animation for free.
        function Library:ToggleKeybindsFrame(Value: boolean?)
            local Opening = Value
            if Opening == nil then
                Opening = not Library.KeybindFrame.Visible
            end
            AnimationJuice.OpenClose(Library.KeybindFrame, Opening, TweenService, AnimationPresets.Macro)
            return Opening
        end
        ...
        -- Restrained wireframe/scanline background texture - the "techy" backdrop.
        ThemeMatrixGrid.Build(New, MainFrame, TweenService)
        ...
        local ResizeIconLabel = New("ImageLabel", {
        ...
        -- Glow-highlight the resize grip on hover so it reads as interactive.
        if WindowInfo.Resizable then
            Library:GiveSignal(ResizeButton.MouseEnter:Connect(function()
                ResizeIconLabel.ImageColor3 = Library.Scheme.AccentColor
                TweenService:Create(ResizeIconLabel, AnimationPresets.Micro, { ImageTransparency = 0 }):Play()
            end))
            Library:GiveSignal(ResizeButton.MouseLeave:Connect(function()
                ResizeIconLabel.ImageColor3 = Library.Scheme.FontColor
                TweenService:Create(ResizeIconLabel, AnimationPresets.Micro, { ImageTransparency = 0.5 }):Play()
            end))
        end
    ...
    function Window:AddDialog(Idx, Info)
        ...
        Library:AddOutline(DialogFrame, true)
        ...
        TweenService:Create(DialogScale, AnimationPresets.Macro, {
            Scale = 1
        }):Play()
        ...
        function Dialog:Dismiss()
            ...
            local CloseTween = TweenService:Create(DialogScale, AnimationPresets.Macro, { Scale = 0.9 })
            TweenService:Create(DialogOverlay, AnimationPresets.Macro, { BackgroundTransparency = 1 }):Play()
            CloseTween:Play()

            task.delay(AnimationPresets.Macro.Time, function()
    ...
    function Window:Toggle(Value: boolean?)
        ...
        -- Scale+pop open/close instead of an instant Visible flip.
        AnimationJuice.OpenClose(MainFrame, Library.Toggled, TweenService, AnimationPresets.Macro)
...
function Library:CreateLoading(LoadingInfo)
    ...
    IntroBootScreen.AddGlowingLeadingEdge(New, SliderFill, CustomImageManager.GetAsset("Glow"), {
        Size = UDim2.fromOffset(14, 14),
    })
    ...
    --// Boot Terminal Log \\--
    local BootTerminalHolder = New("Frame", {
        BackgroundTransparency = 1,
        Size = UDim2.new(0.7, 0, 0, 76),
        Parent = InnerContent,
    })
    local BootTerminal = IntroBootScreen.CreateTerminalLog(New, BootTerminalHolder, {
        MaxLines = 5,
        TextColor = function()
    ...
    Library:GiveSignal(SidebarList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
    ...
    function Loading:UpdateLayout()
        ...
        TweenService:Create(MainFrame, AnimationPresets.Macro, { Size = UDim2.fromOffset(FinalWidth, FinalHeight) }):Play()
        TweenService:Create(SideBar, AnimationPresets.Macro, { Position = UDim2.fromOffset(Loading.ContentWidth, 0), Size = UDim2.new(0, ShowSidebar and Loading.SidebarWidth or 0, 1, 0) }):Play()
        TweenService:Create(Container, AnimationPresets.Macro, { Size = UDim2.new(0, ShowSidebar and Loading.ContentWidth or Loading.WindowWidth, 1, 0) }):Play()
    ...
    function Loading:SetCurrentStep(Step)
        ...
        TweenService:Create(SliderFill, AnimationPresets.Boot, { Size = UDim2.fromScale(Progress, 1) }):Play()
    ...
    function Loading:SetErrorButtons(Buttons)
        ...
        for Idx, ButtonInfo in Buttons do
            ...
            Library:GiveSignal(TextBtn.MouseEnter:Connect(function()
                TweenService:Create(TextBtn, AnimationPresets.Micro, {
                    BackgroundColor3 = HoverColor
                }):Play()
            end))
            Library:GiveSignal(TextBtn.MouseLeave:Connect(function()
                TweenService:Create(TextBtn, AnimationPresets.Micro, {
                    BackgroundColor3 = ActiveColor
                }):Play()
            end))

            Library:GiveSignal(TextBtn.MouseButton1Click:Connect(function()
                if ButtonInfo.Callback then
